import argparse
import os
from cfg.prune_llama import packed_ft_llama_Lamini_config_lora
import torch
...
def main(config):
    ...
    # Load tokenizers
    tokenizer = AutoTokenizer.from_pretrained(config["models"])
    ...
    monkey_patch_packing_for_model(config["models"])
    model = AutoModelForCausalLM.from_pretrained(config["models"], **model_kwargs)
    ...
    if accelerator.is_main_process:
        print(config["models"], "ft is start!")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tokenizer_max', type=int, default=None)
    ...
    if args.tokenizer_max is not None:
        config["tokenizer"]["max_length"] = args.tokenizer_max
